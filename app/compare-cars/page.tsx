...
export default function CompareCarsPage() {
  ...
  const selectedCars = useMemo(
    () => {
      const filtered = slots.map((id) => cars.find((c) => c.id === id)).filter((c): c is Car => Boolean(c));
      return isMobile ? filtered.slice(0, 2) : filtered;
    },
    [slots, isMobile],
  );
        ...
        <section id="compare-result" className="py-14 scroll-mt-24 sm:py-20">
          <div className="container-page">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-6 sm:p-8">
              ...
              <div className="mb-6 overflow-x-auto pb-4 border-b border-slate-100">
                <div
                  className="grid gap-4 sm:gap-6 min-w-full"
                  style={{
                    gridTemplateColumns: `repeat(${selectedCars.length}, minmax(150px, 1fr))`,
                  }}
                >
                  {selectedCars.map((car) => (
                    <div key={car.id} className="text-center flex flex-col items-center justify-between min-w-[140px] sm:min-w-[180px]">
                      <div className="w-full">
                        <div className="relative mx-auto aspect-[4/3] w-full max-w-[180px] overflow-hidden rounded-xl bg-slate-100 sm:max-w-[220px]">
                          <Image src={car.image} alt={`${car.make} ${car.model}`} fill className="object-cover" />
                        </div>
                        <p className="mt-3 text-xs sm:text-sm font-bold uppercase text-slate-900 line-clamp-1" title={`${car.make} ${car.model}`}>
                          {car.make} {car.model}
                        </p>
                        <p className="text-base sm:text-2xl font-extrabold text-slate-900 mt-1 whitespace-nowrap">
                          {formatPrice(car.price)}
                        </p>
                      </div>
                      <div className="mt-3 flex flex-col gap-2 w-full max-w-[160px]">
                        <Link href="/book-a-test-drive" className="btn btn-primary !px-2.5 !py-2 text-[11px] sm:text-xs whitespace-nowrap">
                          Book Test Drive
                        </Link>
                        <Link href={`/pre-owned-cars/${car.id}`} className="btn btn-outline !px-2.5 !py-2 text-[11px] sm:text-xs whitespace-nowrap">
                          View Details
                        </Link>
                      </div>
                    </div>
              ...
              {/* Spec table matrix - responsive with sticky spec column & horizontal scroll */}
                ...
                <table className="w-full min-w-[500px] border-collapse text-xs sm:text-sm">
                  <tbody>
                    {rows.map((row) => {
                      ...
                      return (
                        <tr key={row.label} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/50 transition-colors">
                          <td className="w-28 sm:w-36 p-3 text-[11px] sm:text-xs font-bold uppercase tracking-wider text-slate-500 bg-slate-50/90 backdrop-blur-sm sticky left-0 z-10 sm:static">
                            {row.label}
                          </td>
                          {selectedCars.map((car, i) => (
                            <td
                              key={car.id}
                              style={{ width: `${100 / selectedCars.length}%` }}
                              className={`p-3 font-semibold text-slate-900 min-w-[140px] sm:min-w-[180px] ${
                                i === winnerIdx ? 'text-green-600 bg-green-50/30' : ''
                              }`}
                            >
            ...
            {/* Recommendation */}
            {recommendation && (
              <Reveal className="mt-6 flex flex-col items-center text-center gap-4 rounded-xl bg-brand-red/5 p-6 sm:flex-row sm:items-center sm:justify-between sm:text-left">
                ...
                <Link href="/book-a-test-drive" className="btn btn-primary w-full sm:w-auto shrink-0 justify-center">
                  Book Test Drive
                </Link>
              </Reveal>
            )}
