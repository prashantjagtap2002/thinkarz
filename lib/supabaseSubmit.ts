...
export async function submitToSupabase(payload: Record<string, any>): Promise<boolean> {
  try {
    ...
    if (tableName === 'sell_your_car_valuation_form') {
      ...
      dataToInsert.brand = cleanPayload.brand || cleanPayload.make || null;
      ...
      dataToInsert.phone = cleanPayload.phone || cleanPayload.mobile || null;
      dataToInsert.email = cleanPayload.email || null;
      if (cleanPayload.location) dataToInsert.location = cleanPayload.location;
      if (cleanPayload.notes) dataToInsert.notes = cleanPayload.notes;
      ...
      dataToInsert.offer_price = cleanPayload.offer_price || cleanPayload.offerPrice || cleanPayload.offer || cleanPayload.offer_amount || null;
      dataToInsert.name = cleanPayload.name || cleanPayload.fullName || null;
      dataToInsert.phone = cleanPayload.phone || cleanPayload.mobile || null;
      dataToInsert.email = cleanPayload.email || null;
